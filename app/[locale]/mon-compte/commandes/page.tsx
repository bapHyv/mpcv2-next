...
const Commandes = () => {
  ...
  return (
    <section className="text-gray-800 mt-8 py-12 px-6 max-w-4xl mx-auto rounded-lg shadow-md">
      <div className="mb-12">
        <h3 className="text-2xl font-bold text-teal-800 mb-6">{t("title")}</h3>
        <div className="space-y-10">
          {orders.map((order) => (
            <div key={order.id} className="border border-gray-300 rounded-lg p-6 bg-white shadow-sm">
                ...
                <div className="text-right">
                  ...
                  <p
                    className={`text-sm font-medium ${
                      order.status === "wc-completed" ? "text-emerald-600" : order.status === "wc-on-hold" ? "text-yellow-600" : "text-red-600"
                    }`}
                  >
              ...
              <table className="mt-6 w-full text-gray-700">
                <thead>
                  <tr className="border-b text-left text-sm font-medium text-gray-600">
                    ...
                    <th className="py-3 pr-8 hidden sm:table-cell">{t("order.price")}</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-gray-200">
                  {order.products.map((product: any) => (
                    <tr key={product.name} className="text-sm">
                      <td className="py-4 pr-8 text-teal-900 font-medium">{product.name}</td>

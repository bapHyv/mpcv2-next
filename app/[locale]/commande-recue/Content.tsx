...
export default function Content({ searchParams, locale }: ContentProps) {
  ...
  useEffect(() => {
    ...
    const { failure_reason, payment, status } = searchParams;
    ...
    if (payment === "bankTransfer") {
      ...
      if (status === "success") {
        ...
        addAlert(
          uuid(),
          t("orderReceived.alerts.cardFailed.text", { reason: failure_reason || t("orderReceived.status.unknownReason") }),
  ...
  return (
    <div className="container m-auto px-4 space-y-8 mt-6 2xl:w-2/3 2xl:m-auto">
      ...
      {payment === "secure3dcard" && (
        <div
          className={clsx(
            ...
            searchParams.status === "success" && "bg-emerald-50 text-emerald-800 border border-emerald-200",
            searchParams.status === "fraud_warning" && "bg-yellow-50 text-yellow-800 border border-yellow-200",
            (searchParams.status === "failed" ||
              searchParams.status === "error" ||
              searchParams.status === "bank_refusal" ||
              searchParams.status === "unknown_failure" ||
              searchParams.status === "auth_failed") &&
              "bg-red-50 text-red-800 border border-red-200",
            (searchParams.status === "processing" ||
              searchParams.status === "abandoned" ||
              searchParams.status === "max_attempts" ||
              searchParams.status === "technical_error") &&
              "bg-blue-50 text-blue-800 border border-blue-200"
          )}
        >
          <p className="font-medium">{t(`orderReceived.status.${searchParams.status || "unknown"}`)}</p>
          ...
          {searchParams.status === "fraud_warning" && <p className="mt-1 text-xs">{t("orderReceived.statusMessages.fraudWarningNote")}</p>}
          {searchParams.status !== "success" && searchParams.status !== "fraud_warning" && (
      ...
      {isSuccess && order && cart ? (
        ...
        !isLoading && (
          <section aria-labelledby="payment-failed-heading" className={sectionWrapperClassname}>
            <Title type="h2" title={t("orderReceived.failure.title")} classname={baseTitleClassname} />
            <div className="mt-4 text-center text-gray-600">
              <p>{t("orderReceived.failure.text")}</p>
              <Link
                href={`/${locale}/expedition`}
                // className="mt-6 inline-block w-full rounded-md border border-transparent bg-green px-4 py-3 text-base font-medium text-white shadow-sm hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-offset-2 focus:ring-offset-gray-50 sm:w-auto"
                className={twMerge(buttonClassname, "mt-6")}
              >
                {t("orderReceived.failure.button")}
              </Link>
            </div>
          </section>
        )
